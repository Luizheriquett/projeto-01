...
btnCreateRoom.addEventListener('click', async () => {
  ...
  try {
    ...
    setLobbyError('Não foi possível criar a sala. Tente novamente.');
  } finally {
...
async function enterRoom(id, name, avatar) {
  ...
  // Pede permissão de microfone ANTES de trocar de tela, pra falhar cedo
  // e com uma mensagem clara caso o usuário negue.
  try {
    await media.initMic();
  } catch (err) {
  ...
  const joinRes = await new Promise((resolve, reject) => {
    signaling.on('connect', async () => {
      try {
        const res = await signaling.joinRoom({ roomId: id, name, avatar });
        resolve(res);
      } catch (err) {
        reject(err);
      }
    });
    signaling.socket.once('connect_error', () => reject(new Error('Não foi possível conectar ao servidor.')));
  });
  ...
  vad.on('change', (speaking) => {
    updateParticipant(joinRes.selfId, { speaking });
...
function wireSignalingEvents() {
  ...
  signaling.on('user-screen-share-state', ({ id, sharing }) => {
    ...
    if (sharing && !activeShareId) activeShareId = id; // primeiro a compartilhar ganha destaque
    renderAll();
  });
}
...
function clearScreenShareView() {
  ...
  // se alguém mais ainda estiver compartilhando, promove essa pessoa
...
btnMic.addEventListener('click', () => {
  ...
  if (!next) vad.emit('change', false); // ao mutar, para de sinalizar "falando"
});
...
function startAdaptiveQualityLoop() {
  webrtc.on('stats', ({ lossRatio, rtt }) => {
    if (!media.isSharingScreen()) return;
    if (selectQuality.value !== 'auto') return; // usuário escolheu manualmente

    const width = media.screenVideoTrack?.getSettings().width || 1280;
    let kbps = media.bitrateForWidth(width);

    // perda de pacotes alta ou RTT alto -> reduz bitrate
